...
export default function Reports() {
  ...
  const rawDob = user?.date_of_birth || user?.user_metadata?.date_of_birth
  const formattedDob = formatDateOfBirth(rawDob)
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      try {
        ...
        await api.post('/reports', { pdf_data: pdfBase64 })
      } catch (err) {
        ...
        <div ref={reportRef} style={{ padding: '2rem', backgroundColor: '#FFFFFF', color: '#1E293B' }}>
          ...
          <div style={{ borderBottom: '2px solid #E2E8F0', paddingBottom: '1.5rem', marginBottom: '2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div style={{ textAlign: 'right' }}>
              <p style={{ fontWeight: 600, margin: 0 }}>{user?.name || user?.user_metadata?.name || 'Patient Name'}</p>
              <p style={{ color: '#64748B', fontSize: '0.875rem', margin: 0 }}>
                Date of Birth: {formattedDob}
              </p>
              <p style={{ color: '#64748B', fontSize: '0.875rem', margin: 0 }}>
                Age: {calculatedAge !== null ? `${calculatedAge} years` : 'Not specified'}
              </p>
